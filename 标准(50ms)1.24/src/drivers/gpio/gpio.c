 ...
 *  @note       与port_init不同的是，此函数不需要配置 MUX 复用功能（即使配置了也不生效），
                MUX 保留 为原先寄存器配置的值
 *  Sample usage:       port_init_NoALT (PORTA,8, IRQ_RISING | PF | PULLUP );    //初始化 PTA8 管脚，上升沿触发中断，带无源滤波器，保留原先复用功能，上拉电阻
 */
void  port_init_NoALT(PTxn ptxn, uint32 cfg)
{
    ...
    SIM_SCGC5 |= (SIM_SCGC5_PORTA_MASK << portx );     //开启PORTx端口  PTX(ptxn)

    //清空cfg里的MUX，加载寄存器里的MUX
    cfg &= ~PORT_PCR_MUX_MASK;                      //清了MUX 字段（即不需要配置ALT，保持原来的ALT）
    cfg |=  (PORT_PCR_REG(PORTX[portx], ptn) & PORT_PCR_MUX_MASK);    //读取寄存器里配置的 MUX

    PORT_PCR_REG(PORTX[portx], ptn) = cfg;            // 复用功能 , 确定触发模式 ,开启上拉或下拉电阻
}
